...
// === Scheduled Cron: prune old activity logs ===
export async function scheduled(event, env, ctx) {
  try {
    const days = parseInt(env.LOG_RETENTION_DAYS || "90", 10);
    const cutoff = new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString();
    // Use simple string compare against ISO timestamps
    const stmt = env.ea_license_db_new.prepare("DELETE FROM activity_logs WHERE created_at < ?1");
    const res = await stmt.bind(cutoff).run();
    // Optional: you can log to console; visible in wrangler tail during cron
    var changes = (res && res.meta && (typeof res.meta.changes !== 'undefined')) ? res.meta.changes : 'n/a';
    console.log('[cron] Pruned activity_logs older than ' + days + ' day(s). cutoff=' + cutoff + ' deleted=' + changes);
  } catch (err) {
