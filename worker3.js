...
// === Telegram Alerts (append-only wrapper) ===
async function telegramNotify(env, message) {
  try {
    ...
    const chat  = env.TELEGRAM_CHAT_ID;     // set in [vars] or secret
...
// Wrap original logActivity to add Telegram, without changing its definition above
try {
  const __origLogActivity = logActivity;
  // Reassign the binding to a new function that calls the original then notifies
  logActivity = async function(env, licenseKey, action, details, request) {
    await __origLogActivity(env, licenseKey, action, details, request);
    try {
      var important = { generate: 1, activation: 1, suspension: 1, revocation: 1 };
      if (important[action]) {
        var ip = (request && (request.headers.get('CF-Connecting-IP') || request.headers.get('X-Forwarded-For'))) || 'unknown';
        var prefix = action === 'generate' ? '🎫 New License' :
                     action === 'activation' ? '✅ Activation' :
                     action === 'suspension' ? '⚠️ Suspension' :
                     action === 'revocation' ? '🗑️ Revocation' : 'ℹ️ Event';
        var msg = prefix + ' — ' + licenseKey + '\\n' + details + '\\nIP: ' + ip;
        await telegramNotify(env, msg);
      }
    } catch (e) { try { console.error('notify error:', e); } catch(_) {} }
  };
} catch (e) {
  try { console.error('Unable to wrap logActivity for Telegram alerts:', e); } catch(_) {}
}
