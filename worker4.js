...
// === Telegram Alerts helper ===
async function telegramNotify(env, message) {
  try {
    var token = env.TELEGRAM_BOT_TOKEN;
    var chat  = env.TELEGRAM_CHAT_ID;
    ...
    if (!res.ok) { try { console.warn("telegramNotify failed:", res.status); } catch(_){} }
  } catch (e) {
    try { console.error("telegramNotify error:", e); } catch(_) {}
  }
}


// === Alerting 2.0: wrap activation/verification to alert on failures ===
try {
  if (typeof handleActivation === 'function') {
    const __origHandleActivation = handleActivation;
    handleActivation = async function(request, env, headers) {
      // clone request to read inputs safely
      var clone = request.clone();
      var body = null;
      try { body = await clone.json(); } catch(_) {}
      var resp = await __origHandleActivation(request, env, headers);
      try {
        var text = await resp.clone().text();
        var payload = null;
        try { payload = JSON.parse(text); } catch(_e) {}
        if (payload && payload.success === false) {
          var reason = payload.error || 'Unknown error';
          var key = body && body.license_key ? body.license_key : '(unknown)';
          var ip = (request.headers.get('CF-Connecting-IP') || request.headers.get('X-Forwarded-For') || 'unknown');
          // record failure in logs (so digest can find it)
          if (typeof logActivity === 'function') {
            try { await logActivity(env, key, 'activation_fail', reason, request); } catch(_la) {}
          }
          // immediate Telegram alert
          try { await telegramNotify(env, '❌ Activation failed — ' + key + '\n' + reason + '\nIP: ' + ip); } catch(_tg) {}
        }
      } catch(_){}
      return resp;
    };
  }
} catch(e) { try { console.error('activation wrapper error:', e); } catch(_) {} }

try {
  if (typeof handleVerification === 'function') {
    const __origHandleVerification = handleVerification;
    handleVerification = async function(request, env, headers) {
      var clone = request.clone();
      var body = null;
      try { body = await clone.json(); } catch(_) {}
      var resp = await __origHandleVerification(request, env, headers);
      try {
        var text = await resp.clone().text();
        var payload = null;
        try { payload = JSON.parse(text); } catch(_e) {}
        if (payload && payload.success === false) {
          ...
          var ip = (request.headers.get('CF-Connecting-IP') || request.headers.get('X-Forwarded-For') || 'unknown');
          if (typeof logActivity === 'function') {
            try { await logActivity(env, key, 'verification_fail', reason, request); } catch(_la) {}
          }
          try { await telegramNotify(env, '❌ Verification failed — ' + key + '\n' + reason + '\nIP: ' + ip); } catch(_tg) {}
        }
      } catch(_){}
      return resp;
    };
  }
} catch(e) { try { console.error('verification wrapper error:', e); } catch(_) {} }


// === Daily failure digest (last 24h) ===
async function sendFailureDigest(env) {
  try {
    var hours = parseInt(env.ALERT_DIGEST_LOOKBACK_HOURS || "24", 10);
    var cutoff = new Date(Date.now() - hours * 60 * 60 * 1000).toISOString();

    // Count by action
    var countsStmt = env.ea_license_db_new.prepare(
      "SELECT action, COUNT(*) AS n FROM activity_logs WHERE created_at >= ?1 AND action IN ('activation_fail','verification_fail') GROUP BY action"
    );
    var counts = await countsStmt.bind(cutoff).all();
    var rows = (counts && counts.results) ? counts.results : [];

    // Top reasons
    var reasonsStmt = env.ea_license_db_new.prepare(
      "SELECT details, COUNT(*) AS n FROM activity_logs WHERE created_at >= ?1 AND action IN ('activation_fail','verification_fail') GROUP BY details ORDER BY n DESC LIMIT 5"
    );
    var reasons = await reasonsStmt.bind(cutoff).all();
    var rrows = (reasons && reasons.results) ? reasons.results : [];

    var total = 0, actFail = 0, verFail = 0;
    for (var i=0;i<rows.length;i++){ total += rows[i].n; if (rows[i].action==='activation_fail') actFail=rows[i].n; if (rows[i].action==='verification_fail') verFail=rows[i].n; }
    if (!total) { return; } // nothing to report

    var msg = '📊 24h Failure Digest\n' +
              'Total: ' + total + '\n' +
              'Activation fails: ' + actFail + '\n' +
              'Verification fails: ' + verFail + '\n';

    if (rrows.length) {
      msg += '\nTop reasons:\n';
      for (var j=0;j<rrows.length;j++){
        var det = rrows[j].details || '(none)';
        msg += '• ' + det + ' — ' + rrows[j].n + '\n';
      }
    }

    await telegramNotify(env, msg);
  } catch (e) {
    try { console.error('digest error:', e); } catch(_) {}
  }
}


// === Scheduled digest (02:00 UTC via crons) ===
export async function scheduled(event, env, ctx) {
  try { await sendFailureDigest(env); } catch(_e) {}
}
