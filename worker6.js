...
export default {
  ...
  return await completePaystackPurchase(request, env, corsHeaders, tenantId);
      ...
      // User Login endpoint (public) - ONLY ONE DEFINITION
...
// ============= USER AUTHENTICATION HELPERS ============= (SINGLE DEFINITION)
...
// Guest Checkout (COMPLETE FUNCTION)
async function guestCheckout(request, env, headers, tenantId) {
  ...
  // TODO: Verify payment with Paystack API using your SECRET key
  ...
  await env.ea_license_db_new.prepare(`
    ...
    SET payment_status = 'completed', 
        payment_reference = ?,
        license_key = ?
    WHERE product_id = ? AND customer_email = ? AND payment_status = 'pending'
    ORDER BY created_at DESC LIMIT 1
  `).bind(payment_reference, licenseKey, product_id, customer_email).run();
  
  return new Response(JSON.stringify({
    success: true,
    license_key: licenseKey,
    message: 'Payment processed and license created successfully'
  }), { headers });
}

// Add this new function to process completed payments
async function completePaystackPurchase(request, env, headers, tenantId) {
  const data = await request.json();
  const { payment_reference, product_id, customer_email, customer_name } = data;
  ...
  return new Response(JSON.stringify({
    success: true,
    order_id,
    message: 'Order created! Contact support to complete payment.',
